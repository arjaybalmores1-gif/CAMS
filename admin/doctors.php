...
if (isset($_POST['add_doctor'])) {
    ...
    if ($stmt->execute()) {
        header("Location: doctors.php?status=success&msg=Doctor added successfully!");
    } else {
        header("Location: doctors.php?status=error&msg=Error adding doctor.");
    }
    $stmt->close();
    exit();
}
    ...
    <aside id="sidebar" class="fixed md:static inset-y-0 left-0 z-50 w-64 bg-pink-500 text-white flex flex-col shadow-lg -translate-x-full md:translate-x-0 transition-transform duration-300">
        ...
        <nav class="mt-4 flex-1 px-3">
            <ul class="space-y-1">
                ...
                <li><a href="doctors.php" class="flex items-center bg-pink-600 border-l-4 border-white px-3 py-2.5 rounded-lg transition-all"><i data-lucide="stethoscope" class="w-5 h-5 flex-shrink-0"></i><span class="ml-3 font-medium text-sm whitespace-nowrap">Doctors</span></a></li>
                <li><a href="appointments.php" class="flex items-center hover:bg-pink-600 px-3 py-2.5 rounded-lg transition-all"><i data-lucide="clipboard-check" class="w-5 h-5 flex-shrink-0"></i><span class="ml-3 font-medium text-sm whitespace-nowrap">Appointments</span></a></li>
