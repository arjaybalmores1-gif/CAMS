...
if (isset($_POST['book_appointment']) && $patient_id) {
    ...
    if ($conflict->num_rows > 0) {
        header("Location: patient_portal.php?status=conflict");
        exit();
    }
    ...
    if ($insert) {
        ...
        header("Location: patient_portal.php?status=success");
        exit();
    }

    header("Location: patient_portal.php?status=error");
    exit();
}
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside id="sidebar" class="fixed md:static inset-y-0 left-0 z-50 w-64 bg-pink-500 text-white flex flex-col shadow-lg
        ...
        <nav class="mt-4 flex-1 px-3">
            <ul class="space-y-1">
                <li>
                    <a href="patient_portal.php" class="flex items-center bg-pink-600 border-l-4 border-white px-3 py-2.5 rounded-lg transition-all">
                        <i data-lucide="layout-dashboard" class="w-5 h-5 flex-shrink-0"></i>
                        <span class="ml-3 font-medium text-sm whitespace-nowrap">My Dashboard</span>
                    </a>
                </li>
                <li>
                    <a href="patient_notifications.php" class="flex items-center hover:bg-pink-600 px-3 py-2.5 rounded-lg transition-all">
